...
from tccli.services.mdl import v20200326
from tccli.services.mdl.v20200326 import help as v20200326_help


def doCreateMediaLiveInputSecurityGroup(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateMediaLiveInputSecurityGroup", g_param[OptionsDefine.Version])
        return

    param = {
        "Name": argv.get("--Name"),
        "Whitelist": Utils.try_to_json(argv, "--Whitelist"),
...
def doDescribeMediaLiveChannelLogs(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeMediaLiveChannelLogs", g_param[OptionsDefine.Version])
        return
...
def doDeleteMediaLiveInput(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteMediaLiveInput", g_param[OptionsDefine.Version])
        return
...
def doModifyMediaLiveInput(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyMediaLiveInput", g_param[OptionsDefine.Version])
        return
...
def doDescribeMediaLiveInputs(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeMediaLiveInputs", g_param[OptionsDefine.Version])
        return
...
def doDescribeMediaLiveInputSecurityGroup(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeMediaLiveInputSecurityGroup", g_param[OptionsDefine.Version])
        return
...
def doCreateMediaLiveChannel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateMediaLiveChannel", g_param[OptionsDefine.Version])
        return

    param = {
        "Name": argv.get("--Name"),
        "AttachedInputs": Utils.try_to_json(argv, "--AttachedInputs"),
        "OutputGroups": Utils.try_to_json(argv, "--OutputGroups"),
        "AudioTemplates": Utils.try_to_json(argv, "--AudioTemplates"),
        "VideoTemplates": Utils.try_to_json(argv, "--VideoTemplates"),
...
def doDescribeMediaLiveChannel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeMediaLiveChannel", g_param[OptionsDefine.Version])
        return
...
def doStartMediaLiveChannel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("StartMediaLiveChannel", g_param[OptionsDefine.Version])
        return
...
def doCreateMediaLiveInput(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("CreateMediaLiveInput", g_param[OptionsDefine.Version])
        return
...
def doModifyMediaLiveInputSecurityGroup(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyMediaLiveInputSecurityGroup", g_param[OptionsDefine.Version])
        return
...
def doModifyMediaLiveChannel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("ModifyMediaLiveChannel", g_param[OptionsDefine.Version])
        return
...
def doDeleteMediaLiveChannel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteMediaLiveChannel", g_param[OptionsDefine.Version])
        return
...
def doDescribeMediaLiveChannels(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeMediaLiveChannels", g_param[OptionsDefine.Version])
        return
...
def doDeleteMediaLiveInputSecurityGroup(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DeleteMediaLiveInputSecurityGroup", g_param[OptionsDefine.Version])
        return
...
def doStopMediaLiveChannel(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("StopMediaLiveChannel", g_param[OptionsDefine.Version])
        return
...
def doDescribeMediaLiveInputSecurityGroups(argv, arglist):
    g_param = parse_global_arg(argv)
    if "help" in argv:
        show_help("DescribeMediaLiveInputSecurityGroups", g_param[OptionsDefine.Version])
        return
...
AVAILABLE_VERSIONS = {
     'v' + v20200326.version.replace('-', ''): {"help": v20200326_help.INFO,"desc": v20200326_help.DESC},

}


def mdl_action(argv, arglist):
    if "help" in argv:
        ...
        helpstr = HelpTemplate.SERVICE % {"name": "mdl", "desc": desc, "actions": action_str}
        print(helpstr)
    else:
...
def register_arg(command):
    cmd = NiceCommand("mdl", mdl_action)
    command.reg_cmd(cmd)
...
def parse_global_arg(argv):
    ...
    try:
        if params[OptionsDefine.Version] is None:
            version = config["mdl"][OptionsDefine.Version]
            params[OptionsDefine.Version] = "v" + version.replace('-', '')

        if params[OptionsDefine.Endpoint] is None:
            params[OptionsDefine.Endpoint] = config["mdl"][OptionsDefine.Endpoint]
...
def show_help(action, version):
    ...
    helpmsg = HelpTemplate.ACTION % {"name": action, "service": "mdl", "desc": desc, "params": docstr}
    print(helpmsg)

...
class ActionCommand(BaseCommand):
    # To be compatible with legacy cli versions
    # ARG_TYPES = {
    #     'Array': ListArgument,
    ...
    def _call_with_depth_guard(self, args, parsed_globals):
        """Dedicated entry for self-referencing actions: orphan-key filtering, deep-nesting extraction, and depth-limit checks."""
    ...
    def _build_recursive_hint(self, remaining):
        """Build hint text for unknown options that hit a self-reference truncation prefix, pointing to --cli-input-json file://."""
        # Only meaningful under --cli-unfold-argument mode.
        if self._call_mode != Options_define.CliUnfoldArgument:
            return ""
        try:
        ...
        # Collect all leaf prefixes truncated by self-reference, e.g.:
        #   "RuleList.RuleDetail.Children.0" -> "AllocationRuleExpression"
        truncated = self._collect_truncated_prefixes(unfold_params)
        if not truncated:
            return ""

        matched = OrderedDict()  # matched invalid options -> (truncation prefix, self-referencing type name)
    ...
    def _collect_truncated_prefixes(unfold_params):
        """Collect self-reference truncation leaf prefixes from the get_unfold_param_info result.

        :return: ``OrderedDict[prefix_key, type_name]`` containing only items whose recursive_truncated is True.
        """
    ...
    def _prefilter_orphan_keys(args, action_parser):
        """Scan args before argparse parsing to detect value-less --key options and raise an error."""
        if not args:
            return
        valueless_opts = set()

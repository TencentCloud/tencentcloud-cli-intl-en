# -*- coding: utf-8 -*-
"""
recursive-nesting-30: unit tests for document_handler.

Coverage goals:
  1. doc_help / sub-method behavior of the three classes CLIDocumentHandler /
     ServiceDocumentHandler / ActionDocumentHandler;
  2. correct rendering of the recursive hint placeholder on self-reference truncation (members is a string);
  3. branches of _json_format / _handle_object_members / _complex_object_doc / _unfold_complex_object
     across the four members shapes: Array / Object / base type / truncation placeholder;
  4. example / input_example / output_example / various getters.
"""
...
# ============================================================
# Helper: build a fake doc object that accumulates all writes into lines
# ============================================================
...
# ============================================================
# Helper: build a stub Loader that injects return values on demand
# ============================================================
...
def _patch_loader(handler, **overrides):
    """Replace the handler's _cli_data with a stub."""
...
def test_A6_cli_available_services_detailed():
    ...
    # svc-b has no description; it must not crash and should still complete new_line
...
# ============================================================
# C. ActionDocumentHandler basic methods
# ============================================================
...
def test_C3_action_options_brief():
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    # the last key is popped and inserted at the front -- 3 keys here verify the ordering behavior
    _patch_loader(
        h,
...
# ============================================================
# D. _json_format / _handle_object_members branches
# ============================================================
def test_D1_json_format_array_of_base_type_top_level():
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    # indentation == 0 takes the doc_description branch
...
def test_D2_json_format_array_of_base_type_indented():
    ...
    h._json_format({"type": "Array", "members": ["string"]})
    flat = _flat(doc)
    # indentation > 2 takes the doc.write branch
    assert "[string ...]" in flat


def test_D3_json_format_array_recursive_truncation():
    """D3: Array self-reference truncation -> render the recursive hint placeholder."""
    doc = _FakeDoc()
    h = _make_action_handler(doc)
...
def test_D6_json_format_object_base_type_returns_quietly():
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    # type != Array and members is a BASE_TYPE element -> return directly, write nothing
    h._json_format({"type": "string", "members": "string"})
    assert doc.lines == []


def test_D7_json_format_object_recursive_truncation():
    """D7: non-Array self-reference truncation -> render the recursive hint placeholder."""
    doc = _FakeDoc()
    h = _make_action_handler(doc)
...
def test_E1_unfold_complex_object_skips_base_type():
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    # non-Array and members is BASE_TYPE -> return directly
    h._unfold_complex_object({"type": "string", "members": "string"})
    assert doc.lines == []


def test_E2_unfold_complex_object_skips_recursive_truncation():
    """E2: non-Array self-reference truncation (members=type-name string) should return directly."""
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    h._unfold_complex_object({"type": "Node", "members": "Node"})
    # should not write the JSON Syntax title
    flat = _flat(doc)
...
def test_E5_complex_object_doc_skips_recursive_truncation():
    """E5: members is a type-name string (self-reference truncation) -> return directly, no recursion."""
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    h._complex_object_doc({"members": "Node"}, "Available Parameters")
    # should not enter indent / sub-member traversal
    assert doc.lines == []
...
def test_E7_complex_object_doc_skips_recursive_subfield():
    """E7: when a sub-field's members is a type-name string, stop recursing (to avoid self-reference loops)."""
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    members = {
        "Self": {
            ...
            "required": "Optional",
            "document": "self ref",
            "members": "Node",  # <- self-reference placeholder
        }
    }
    h._complex_object_doc({"members": members}, "Available Parameters")
    flat = _flat(doc)
...
def test_G1_base_parameter_empty_writes_none():
    doc = _FakeDoc()
    h = _make_action_handler(doc)
    _patch_loader(
        h,
...
# ============================================================
# I. doc_help end-to-end (detail=True and detail=False)
# ============================================================
def test_I1_action_doc_help_detail_false_skips_description_and_example():
    ...
    h.doc_help(detail=False)
    flat = _flat(doc)
    # detail=False skips description and example
